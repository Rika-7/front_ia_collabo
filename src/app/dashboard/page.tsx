...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Type-safe generic function for grouping
    const formatGrouped = <T,>(
      ...
      rows.forEach((row) => {
        ...
        const countStr = String(row[countField] || "0");
    ...
    // Process company data with proper typing
    const companyMonthData = Object.values(
      filteredCompany.reduce<
        Record<string, { month: string; companies: Set<string>; cases: number }>
      >((acc, row) => {
        const month = row["月"];
    ...
    setCompanyData(companyMonthData);
    ...
    const keywordItems: WordCloudItem[] = Object.entries(keywordMap).map(
      ([text, value]) => ({ text, value })
    );
    setKeywordData(keywordItems);

    // Process university data with proper typing
    const universityMonthData = Object.values(
      filteredUniversity.reduce<
        Record<
          string,
          {
            month: string;
      ...
      .map((d) => ({
        month: d.month,
        researchers: d.researchers.size,
        access: d.access,
        match: d.match,
      }))
      .sort((a, b) => a.month.localeCompare(b.month));

    setUniversityData(universityMonthData);
    ...
    const budgetItems: ChartData[] = Object.entries(budgetMap).map(
      ([name, value]) => ({ name, value })
    );
    setBudgetHist(budgetItems);
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <ChartCard title="登録案件数の推移">
            <BarChart data={companyData}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="cases" fill="#82ca9d" name="案件数" />
            </BarChart>
          </ChartCard>
          <ChartCard title="業種別案件分布">
            <BarChart data={industryDist} layout="vertical">
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="value" fill="#ffc658" name="案件数" />
            </BarChart>
          </ChartCard>
          <ChartCard title="依頼企業ランキング（件数順）">
            <BarChart data={topCompanies} layout="vertical">
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="value" fill="#8884d8" name="案件数" />
            </BarChart>
          </ChartCard>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <ChartCard title="マッチング数推移">
            <BarChart data={universityData}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="match" fill="#ffc658" name="マッチング数" />
            </BarChart>
          </ChartCard>
          <ChartCard title="分野別研究者数">
            <BarChart data={fieldDist} layout="vertical">
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="value" fill="#8884d8" name="研究者数" />
            </BarChart>
          </ChartCard>
          ...
          <ChartCard title="希望職位ランキング">
            <BarChart data={positionRank} layout="vertical">
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="value" fill="#ffc658" name="件数" />
            </BarChart>
          </ChartCard>
        </div>
      </main>
